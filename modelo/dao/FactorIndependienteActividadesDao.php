...
class FactorIndependienteActividadesDao
{
    private $conexion;
    ...
    public function insertIdentificadorActividades($nombreTablaActividades, $id_hidden_table_maq, $kamatiTotalMaquinaria, $totalClienteMaquinaria)
    {
        try {
            session_start();
            ...
            $sql = "INSERT INTO identificadoractividades (id_cotizacionesComercial_fk, nombreTablaActividades, checkFactoresIndependientes, totalKamatiActividades, totalClienteActividades) 
                VALUES (?, ?, ?, ?, ?)";
            ...
            $statement->bind_param('isidd', $_SESSION['id_cotizacion'], $nombreTablaActividades, $id_hidden_table_maq, $kamatiTotalMaquinaria, $totalClienteMaquinaria);
    ...
    public function createFactoresIndependientesActividades($id_IdentificadorMaquinaria_FK, $factor_nombre_indMaquinaria, $factor_valor_indMaquinaria)
    {
        $conn = null;
        $statement = null;
        ...
        try {
            ...
            $conn = $this->conexion->conectarBD();
            $sql = "INSERT INTO factoresindependientesactividades (id_IdentificadorActividades_fk, factorActividadesIndependiente, factorActividadesIndependienteValor)
                VALUES (?, ?, ?)";
            $statement = $conn->prepare($sql);
    ...
    public function registrarFactoresIndependientesActividades($id_Identificador, $factors)
    {
        session_start();
        if (isset($_SESSION['id_cotizacion'])) {
            ...
            foreach ($factors as $factor) {
                ...
                if (!empty($factor_nombre) && !is_null($factor_valor)) {
                    $insertedId = $this->createFactoresIndependientesActividades($id_Identificador, $factor_nombre, $factor_valor);
    ...
    public function insertDataViaticosActividadesIndependientes($id_IdentificadorActividades, $nombreViatico, $valorViatico) {
        try {
            $conn = $this->conexion->conectarBD();
            $sql = "INSERT INTO viaticosactividadesindependiente 
                    (id_IdentificadorActividades_fk2, nombreViaticoActividadesIndependiente, valorViaticoActividadesIndependiente) 
                    VALUES (?, ?, ?)";
            
            $stmt = $conn->prepare($sql);
            $stmt->bind_param("isi", $id_IdentificadorActividades, $nombreViatico, $valorViatico); // "isi" -> int, string, int
            
            if ($stmt->execute()) {
                $insertedId = $conn->insert_id; // Devuelve el ID generado
                $stmt->close();
                $conn->close();
                return $insertedId;
            } else {
            ...
            error_log("Error en la inserción de datos de viáticos: " . $e->getMessage());
            return null;
        }
    }

    public function insertDataTarifaCargoActividadesIndependientes($id_IdentificadorActividades, $nombreViatico, $valorViatico) {
        try {
            $conn = $this->conexion->conectarBD();
            $sql = "INSERT INTO cargosindependientesactividades 
                    (id_IdentificadorActividades_fk3, nombreCargoActividadesIndependiente, valorCargoActividadesIndependiente) 
                    VALUES (?, ?, ?)";
