...
function addNewRow(tbody) {
    ...
    function saveTable() {
        ...
        const originalTableContainer = document.querySelector('#tablaMateriales');
        ...
        // Actualizar los valores en los inputs clonados
        clonedTableContainer.querySelector('input[name="nombre_materiales"]').value = tableName;
        clonedTableContainer.querySelector('input[name="txt_factor_mo1"]').value = factorMo;
        clonedTableContainer.querySelector('input[name="txt_factor_o1"]').value = factorO;
        clonedTableContainer.querySelector('input[name="txt_poliza1"]').value = poliza;
        clonedTableContainer.querySelector('input[name="txt_siemens1"]').value = siemens;
        clonedTableContainer.querySelector('input[name="txt_pilz1"]').value = pilz;
        clonedTableContainer.querySelector('input[name="txt_rittal1"]').value = rittal;
        clonedTableContainer.querySelector('input[name="txt_phoenix1"]').value = phoenix;
        clonedTableContainer.querySelector('input[name="txt_total_kamati"]').value = totalKamati;
        clonedTableContainer.querySelector('input[name="txt_total_cliente"]').value = totalCliente;
    
        // Actualizar el costo Kamati Unitario en las filas clonadas
        clonedTableContainer.querySelectorAll('tbody tr').forEach(row => {
            const precioLista = parseFloat(row.querySelector('textarea[name="precio_lista"]').value.replace(/[^\d,.-]/g, '').replace(',', '.')) || 0;
            const costoKamatiUnitario = precioLista * factorMo * factorO * poliza + siemens + pilz + rittal + phoenix;
            row.querySelector('textarea[name="costo_kamati_unitario"]').value = costoKamatiUnitario.toFixed(2);
    
            const deleteButton = row.querySelector('.delete-btn');
            deleteButton.addEventListener('click', () => showDeleteModal(row));
        });
    
        // Asignar event listeners al botón de agregar filas de la tabla clonada
        const newAddRowButton = clonedTableContainer.querySelector('button[name="button_nueva_fila_materiales"]');
        newAddRowButton.addEventListener('click', () => addNewRow(clonedTableContainer.querySelector('tbody')));
        ...
        // Asignar formateo de moneda a los selects de la tabla clonada
        clonedTableContainer.querySelectorAll('select').forEach(select => {
            select.addEventListener('change', () => {
                updateCurrencyFormatting(select);
                calculateTotals();
            });
        ...
        // Asignar event listeners a los inputs de la tabla clonada
        clonedTableContainer.querySelectorAll('textarea, input').forEach(input => {
            input.addEventListener('input', () => {
        ...
        // Añadir el contenedor clonado al documento
    ...
    function assignDeleteEventListeners() {
        ...
        const names = ['txt_factor_mo1', 'txt_factor_o1', 'txt_poliza1', 'txt_siemens1', 'txt_pilz1', 'txt_rittal1', 'txt_phoenix1', 'txt_identificacion_usd1', 'txt_identificacion_eur1'];
        names.forEach(name => {
