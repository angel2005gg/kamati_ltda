...
export function assignDeleteEventListeners(deleteModal, divContainer, insertId = null) {
    // Asegúrate de que tableBody esté definido

    const tableBody = divContainer.querySelector('.table_materialesClass .tbody_materialesClas'); // Ajusta el selector según tu estructura de HTML
    ...
    deleteButtons.forEach(button => {
        ...
        button.addEventListener('click', () => {
            showDeleteModal(row, deleteModal, divContainer, tableBody, insertId);
        });
    });
    ...
    tableBody.querySelectorAll('.cost-kamati-input, .value-total-input, .precio-lista-input, .descuento-input, .descuento-adicional-input, .cantidad-material, .abreviatura-lista, .select_trm_nu, .date_input_entrega, .inputNewFactorClas, .inputHiddenNewFactorClas, .hiddenInputFactoresIndependientesClas').forEach(input => {
        input.addEventListener('input', () => {
            const row = input.closest('tr');
            updateCostoKamatiUnitario(row, divContainer, tableBody);
        });
    });
    ...
    names.forEach(name => {
        ...
        inputs.forEach(input => {
            input.addEventListener('input', () => {
                ...
                rows.forEach(row => updateCostoKamatiUnitario(row, divContainer, tableBody));
            });
        });
    });
