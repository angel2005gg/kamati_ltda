...
export function updateCostoKamatiUnitarioMaquinaria(row = null, divContainer, tableBody) {
    ...
    container.querySelectorAll('#id_fecha_tiempo_entregaMaquinaria').forEach(input => {
        input.addEventListener('change', function () {
            ...
            const textarea = this.closest('tr').querySelector('#valor_tiempo_entregaMaquinaria');
    ...
    if (precioListaInput && descuentoInput && descuentoAdicionalInput && costKamatiInput && cantidadMaterialSinFormato) {
        ...
        let inputHiddenNewFactor = container.querySelector('.inputHiddenNewFactorMaquinariaClas'); // Usar clase en lugar de ID
        let newFactorValue = container.querySelector('.inputNewFactorMaquinariaClas');
        let hiddenInputFactoresIndependientes = divContainer.querySelector('.hiddenInputFactoresIndependientesClasMaquinaria'); // Input para verificar factores independientes

        // Obtener los factores independientes
        let factorMOHidden = divContainer.querySelector('.factor_MoClassMateriales');
        let factorVHidden = divContainer.querySelector('.factor_VClassMateriales');
        ...
        let factorViaticosValue = viaticos ? parseFloat(viaticos.value) || 0 : 0;
        ...
        const factorVAc = (!newFactor && inputHiddenChecked) ? parseFloat(factorVHidden.value) || 0 : (newFactor ? parseFloat(newFactorValue.value) || 0 : factorViaticosValue);
    ...
    calculateTotalsMaquinaria(divContainer, tableBody);
}
