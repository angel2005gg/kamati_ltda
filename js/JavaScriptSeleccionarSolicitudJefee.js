document.addEventListener('DOMContentLoaded', function() {
    ...
    // Filtrar usuarios
    document.getElementById('fechaEnviadasJefe').addEventListener('change', function() {
        let fecha = this.value;
    
        let xhr = new XMLHttpRequest();
        xhr.open('POST', '../procesar/procesarFiltroSolicitudJefe.php', true);
        xhr.setRequestHeader('Content-type', 'application/x-www-form-urlencoded');
    
        xhr.onload = function() {
            if (xhr.status === 200) {
                let response = JSON.parse(xhr.responseText);
                let table_body_select_pendiente = document.getElementById('table_body_select_jefe');
        ...
        let postData = fecha ? 'fechaJefe=' + encodeURIComponent(fecha) : '';
    
        xhr.send(postData);
    });

    // Obtener detalles del usuario
    function obtenerDetallesUsuario(idPermiso) {
        let xhr = new XMLHttpRequest();
        ...
        xhr.send('id_permiso=' + encodeURIComponent(idPermiso));
    }
});