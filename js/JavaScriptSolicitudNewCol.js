$(document).ready(function() {
    ...
    function handleRadioChange() {
        ...
        if (selectedValue === 'Vacaciones') {
            mostrarAlerta('warning', 'Selección de vacaciones.');
        ...
        if (selectedValue === 'Permiso' && selectedValueRadio2 === 'horas') {
            if(valor >= 16){
                mostrarAlerta('warning', 'Permiso de 16 horas o mas requiere ser licencia.');
                $('#licencia').prop('checked', true);
            }
        } else if(selectedValue === 'Permiso' && selectedValueRadio2 === 'dias'){
            if(valor > 2){
                mostrarAlerta('warning', 'Permiso de mas de 2 dias requiere ser licencia.');
                $('#licencia').prop('checked', true);
            }
            
        } else if(selectedValue === 'Licencia' && selectedValueRadio2 === 'horas'){
            if(valor < 16){
                mostrarAlerta('warning', 'Licencia de menos de 16 horas requiere ser permiso.');
                $('#permiso').prop('checked', true);
            }
        } else if(selectedValue === 'Licencia' && selectedValueRadio2 === 'dias'){
            if(valor < 3){
                mostrarAlerta('warning', 'Licencia de menos de 2 dias requiere ser permiso.');
                $('#permiso').prop('checked', true);
            }
        }
    ...
    $('#diascompensados').on('change', function() {
        ...
        if (selectedValue === 'Si') {
            mostrarAlerta('warning', 'Elegiste compensar tus vacaciones.');
            $('#cantidad_dias').prop('disabled', false);

        } else if(selectedValue === 'No'){
            mostrarAlerta('warning', 'Elegiste no compensar tus vacaciones.');
            $('#cantidad_dias').prop('disabled', true).val('');
        }
    });
