document.addEventListener('DOMContentLoaded', function() {
    ...
    // Filtrar usuarios
    document.getElementById('fechaEnviadasL').addEventListener('change', function() {
        let fecha = this.value;
    
        let xhr = new XMLHttpRequest();
        xhr.open('POST', '../procesar/procesarFiltroSolicitudd.php', true);
        xhr.setRequestHeader('Content-type', 'application/x-www-form-urlencoded');
    
        xhr.onload = function() {
            if (xhr.status === 200) {
                let response = JSON.parse(xhr.responseText);
                let table_body_select_pendiente = document.getElementById('table_body_select');
        ...
        let postData = fecha ? 'fechaEnviada=' + encodeURIComponent(fecha) : '';
