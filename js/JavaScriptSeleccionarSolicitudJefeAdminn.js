document.addEventListener('DOMContentLoaded', function() {
    ...
    document.getElementById('fechaEnviadasJefeAdmin').addEventListener('change', function() {
        let fecha = this.value;
    
        let xhr = new XMLHttpRequest();
        xhr.open('POST', '../procesar/procesarFiltroSolicitudJefeAdmin.php', true);
        xhr.setRequestHeader('Content-type', 'application/x-www-form-urlencoded');
    
        xhr.onload = function() {
            if (xhr.status === 200) {
                let response = JSON.parse(xhr.responseText);
                let table_body_select_pendiente = document.getElementById('table_body_select_adminJefe');
        ...
        let postData = fecha ? 'fechaJefeAdmin=' + encodeURIComponent(fecha) : '';
