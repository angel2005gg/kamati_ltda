document.addEventListener('DOMContentLoaded', function() {
    ...
    document.getElementById('txt_filtro_solicitud_pendiente').addEventListener('input', function() {
        let nombrePermiso = this.value;
    
        let xhr = new XMLHttpRequest();
        xhr.open('POST', '../procesar/procesarFiltroSolicitudPendiente.php', true);
        xhr.setRequestHeader('Content-type', 'application/x-www-form-urlencoded');
    
        xhr.onload = function() {
            if (xhr.status === 200) {
                let response = JSON.parse(xhr.responseText);
                let table_body_select_pendiente = document.getElementById('table_body_select_pendiente');
    
                table_body_select_pendiente.innerHTML = '';
                ...
                response.forEach(function(row) {
                    ...
                    `;
                    table_body_select_pendiente.appendChild(tr);
                });
        ...
        let postData = nombrePermiso ? 'nombreEstado=' + encodeURIComponent(nombrePermiso) : '';
    ...
    function obtenerDetallesUsuario(id_Permiso) {
        ...
        xhr.onload = function() {
            if (xhr.status === 200) {
                ...
                document.getElementById('txt_Remuneracion').value = response.remuneracion;
                document.getElementById('txt_Remuneracion1').value = response.remuneracion;
                document.getElementById('txt_hidden').value = response.id_Permisos;
                document.getElementById('txt_hidden_estado').value = response.estado_permiso;
                document.getElementById('txt_hidden_user_id_jefe').value = response.id_jefe_area;
