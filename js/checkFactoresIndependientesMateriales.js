...
export async function checkFactoresIndependientesMaterialess(baseRow, divContainer, tableBody) {
    // Seleccionamos los elementos dentro del contenedor dinámico
    const toggleCheckbox = divContainer.querySelector('.hidden_checkBoxMaterialesClass');
    const toggleCheckboxHidden = divContainer.querySelector('.hiddenInputFactoresIndependientesClasess');
    const divWithHidden = divContainer.querySelector('.divHiddenMaterialesClass');
    const inputHiddenDivHidden = divContainer.querySelector('.hiddenInputFactoresIndependientesClas');
    ...
    if (toggleCheckbox && divWithHidden) {
        ...
        if (toggleCheckboxHidden.value === "1" && !toggleCheckbox.checked) {
            ...
            updateCostoKamatiUnitario(baseRow, divContainer, tableBody); // Llamamos a la función para actualizar los costos
        }
