export async function saveSingleRowDataMaquinaria(newRow, insertId) {
    try {
        // Formato de número
        ...
        const response = await fetch('../phpServer/insertDataFilaMaquinariaAdicional.php', {
            method: 'POST',
        ...
        if (!result.success) {
            ...
            insertedIds.forEach((id, index) => {
                const clonedRow1 = newRow.querySelectorAll(".id_fila_MaquinariaTable_class")[index];
                if (clonedRow1) {
                    clonedRow1.value = insertId; // Asigna el insertId a la fila correspondiente
                }
                const clonedRow = newRow.querySelectorAll(".id_fila_MaquinariaTable_class_clonedUpdate")[index];
                if (clonedRow) {
