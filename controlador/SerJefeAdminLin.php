                ...
                if ($_SERVER['REQUEST_METHOD'] === 'POST') {
                    ...
                    foreach ($campos as $campo) {
                        if (!isset($_POST[$campo]) || empty($_POST[$campo])) {
                            header("Location: ../vista/ConsultaUsuariosJefeAdminLin.php?tipo=danger&mensaje=" . urlencode('Uno de los campos esta vacío'));
                            exit;
                        }
                    }
                    ...
                    if ($control->controlUpdateUser($users)) {
                        ...
                        header("Location: ../vista/ConsultaUsuariosJefeAdminLin.php?tipo=success&mensaje=" . urlencode('Actualización exitosa.'));
                        exit;
                    } else {
                        header("Location: ../vista/ConsultaUsuariosJefeAdminLin.php?tipo=danger&mensaje=" . urlencode('Actualización fallida'));
                        exit;
                    }
                } else {
                    header("Location: ../vista/ConsultaUsuariosJefeAdminLin.php?tipo=danger&mensaje=" . urlencode('Método no permitido'));
                    exit;
                }
            } catch (Exception $e) {
                header("Location: ../vista/ConsultaUsuariosJefeAdminLin.php?tipo=danger&mensaje=" . urlencode('Actualización fallida'));
                ...
                if ($_SERVER['REQUEST_METHOD'] === 'POST') {
                    if ($_POST['hiddenContrasenaVerificacion'] === "1") {
                        ...
                        if ($_POST['txt_contrasena'] !== $_POST['txt_verificarContrasena']) {
                            header("Location: ../vista/ConsultaUsuariosJefeAdminLin.php?tipo=danger&mensaje=" . urlencode('Las contraseñas no coinciden.'));
                            exit;
                        } else if (empty($_POST['txt_contrasena']) && empty($_POST['txt_verificarContrasena'])) {
                            header("Location: ../vista/ConsultaUsuariosJefeAdminLin.php?tipo=danger&mensaje=" . urlencode('No puedes actualizar una contraseña vacía.'));
                            exit;
                        } else {
                            $control = new ControladorUser();
                            ...
                            header("Location: ../vista/ConsultaUsuariosJefeAdminLin.php?tipo=success&mensaje=" . urlencode('Actualización de contraseña exitosa.'));
                            exit;
                        }
                    } else if ($_POST['hiddenContrasenaVerificacion'] === "0") {
                        header("Location: ../vista/ConsultaUsuariosJefeAdminLin.php?tipo=danger&mensaje=" . urlencode('Actualización de contraseña fallida.'));
                        exit;
                    }
                } else {
                    header("Location: ../vista/ConsultaUsuariosJefeAdminLin.php?tipo=danger&mensaje=" . urlencode('Actualización de contraseña fallida.'));
