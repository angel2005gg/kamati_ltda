...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
                ...
                if (isset($_POST['radio1'])) {
                    ...
                    try {
                        ...
                        if ($_POST['radio1'] === 'Cumpleaños' || $_POST['radio1'] === 'DiaFamilia') {
                            ...
                            foreach ($campos as $campo) {
                                if (!isset($_POST[$campo]) || empty($_POST[$campo])) {
                                    //ALERT
                                    header("Location: ../vista/solicitarPermiso.php?tipo=danger&mensaje=" . urlencode('Uno de los campos esta vacío.'));
                                    exit;
                                }
                            }
                        ...
                        if ((DateTime::createFromFormat('Y-m-d', $_POST["txt_date_eleboracion1"]) !== false) && (DateTime::createFromFormat('Y-m-d', $_POST["date_inicio"]) !== false) && (DateTime::createFromFormat('Y-m-d', $_POST["date_fin"]) !== false)) {
                            ...
                            switch ($_POST['radio1']) {
                                case 'Permiso':
                                    if ($permisos !== null) {
                                        ...
                                        //ALERT 
                                        header("Location: ../vista/solicitarPermiso.php?tipo=danger&mensaje=" . urlencode('Tu solicitud no ha sido ennvíada.'));
                                        exit;
                                    }
                                    break;
                                case 'Licencia':
                                ...
                                case 'Vacaciones':
                                    if ($permisos !== null) {
                                        ...
                                        //ALERT 
                                        header("Location: ../vista/solicitarPermiso.php?tipo=danger&mensaje=" . urlencode('Tu solicitud no ha sido envíada.'));
                                        exit;
                                    }
                                    break;
                                default:
                            ...
                            //ALERT FECHA
                            header("Location: ../vista/solicitarPermiso.php?tipo=danger&mensaje=" . urlencode('Las fechas no estan bien.'));
                            exit;
                        }
                    } catch (Exception $e) {
                        //ALERT
                        header("Location: ../vista/solicitarPermiso.php?tipo=danger&mensaje=" . urlencode('Tu solicitud no se ha enviado.'));
                        exit;
                    }
                } else {
                    //ALERT 
                    header("Location: ../vista/solicitarPermiso.php?tipo=danger&mensaje=" . urlencode('Debes seleccionar un tipo de solicitud primero.'));
                    exit;
                }
                break;
    ...
    header("Location: ../vista/solicitarPermiso.php?tipo=danger&mensaje=" . urlencode('Método no válido.'));
    exit;
}
