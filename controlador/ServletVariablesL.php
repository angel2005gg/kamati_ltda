    ...
    if ($menu === 'variables') {
        if (isset($_POST['btn_update_num1'])) {
            try {
                ...
                if ($horaSemanal != null && $_POST['txt_nombre_cargo0'] != 0) {
                    ...
                    if ($variables != null) {
                        ...
                        header("Location: ../vista/variablesL.php?tipo=success&mensaje=" . urlencode('Actualización de horas semanales exitosa.'));
                        exit;
                    } else {
                        header("Location: ../vista/variablesL.php?tipo=danger&mensaje=" . urlencode('Actualización fallida.'));
                        exit;
                    }
                } else {
                    //alert vacio
                    header("Location: ../vista/variablesL.php?tipo=danger&mensaje=" . urlencode('No se puede registrar cero.'));
                    exit;
                }
            } catch (Exception $e) {
                header("Location: ../vista/variablesL.php?tipo=danger&mensaje=" . urlencode('Actualización fallida.'));
            ...
            if ($salarioMensual != null && $_POST['txt_nombre_cargo1'] != 0) {
                ...
                if ($variables != null) {
                    $variablesDao->updateVariables($variables);
                    header("Location: ../vista/variablesL.php?tipo=success&mensaje=" . urlencode('Actualización exitosa del salario mínimo.'));
                    exit;
                } else {
                    header("Location: ../vista/variablesL.php?tipo=danger&mensaje=" . urlencode('Actualización fallida.'));
                    exit;
            ...
            if ($horasDia != null && $_POST['txt_nombre_cargo2'] != 0) {
                ...
                if ($variables != null) {
                    $variablesDao->updateVariables($variables);
                    header("Location: ../vista/variablesL.php?tipo=success&mensaje=" . urlencode('Actualización de horas díarias exitosa.'));
                    exit;
                } else {
                    header("Location: ../vista/variablesL.php?tipo=danger&mensaje=" . urlencode('Actualización fallida.'));
                    exit;
