                ...
                if ($_SERVER['REQUEST_METHOD'] === 'POST') {
                    ...
                    foreach ($campos as $campo) {
                        if (!isset($_POST[$campo]) || empty($_POST[$campo])) {
                            //ALERT CAMPOS VACÍOS
                            header("Location: ../vista/ConsultaUsuarios.php?tipo=danger&mensaje=" . urlencode('Uno de los campos esta vacío.'));
                            exit;
                        }
                    }
                    ...
                    if ($users !== null) {
                        ...
                        header("Location: ../vista/ConsultaUsuarios.php?tipo=success&mensaje=" . urlencode('Actualización exitosa.'));
                        exit;
                    } else {
                        //ALERT ACTUALIZACION FALLIDA
                        header("Location: ../vista/ConsultaUsuarios.php?tipo=danger&mensaje=" . urlencode('Actualización fallida.'));
                        exit;
                    }
                } else {
                    //ALERT METODO NO VÁLIDO
                    header("Location: ../vista/ConsultaUsuarios.php?tipo=danger&mensaje=" . urlencode('Método no válido.'));
                    exit;
                }
            } catch (Exception $e) {
                //ALERT ERROR
                ...
                if ($_SERVER['REQUEST_METHOD'] === 'POST') {
                    if ($_POST['hiddenContrasenaVerificacion'] === "1") {
                        ...
                        if ($_POST['txt_contrasena'] !== $_POST['txt_verificarContrasena']) {
                            header("Location: ../vista/ConsultaUsuarios.php?tipo=danger&mensaje=" . urlencode('Las contraseñas no coinciden.'));
                            exit;
                        } else if (empty($_POST['txt_contrasena']) && empty($_POST['txt_verificarContrasena'])) {
                            header("Location: ../vista/ConsultaUsuarios.php?tipo=danger&mensaje=" . urlencode('No puedes actualizar una contraseña vacía.'));
                            exit;
                        } else {
                            $control = new ControladorUser();
                            ...
                            header("Location: ../vista/ConsultaUsuarios.php?tipo=success&mensaje=" . urlencode('Actualización de contraseña exitosa.'));
                            exit;
                        }
                    } else if ($_POST['hiddenContrasenaVerificacion'] === "0") {
                        header("Location: ../vista/ConsultaUsuarios.php?tipo=danger&mensaje=" . urlencode('Actualización de contraseña fallida.'));
                        exit;
                    }
                } else {
                    header("Location: ../vista/ConsultaUsuarios.php?tipo=danger&mensaje=" . urlencode('Actualización de contraseña fallida.'));
